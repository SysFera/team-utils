...
function(module) {
	...
	function plurality(value, unit) {
		return value+' '+((value === 1) ? LABELS_SINGULAR[unit] : LABELS_PLURAL[unit]);
	}
	...
	var setLabels = countdown.setLabels = function(singular, plural) {
		...
		for (var i=LABEL_MILLISECONDS; i<=LABEL_MILLENNIA; i++) {
			...
			LABELS_SINGULAR[i] = singular[i] || LABELS_SINGULAR[i];
	...
	var resetLabels = countdown.resetLabels = function() {
		LABELS_SINGULAR = 'milliseconde|seconde|minute|heure|jour|semaine|mois|an|décade|siècle|millénaire'.split('|');
		LABELS_PLURAL = 'millisecondes|secondes|minutes|heures|jours|semaines|mois|ans|décades|siècles|millénaires'.split('|');
	};
